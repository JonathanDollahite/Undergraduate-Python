...
class App(threading.Thread):
    ...
    def __init__(self, bot_name="Too Lazy to Name Me"):
        self.bot_name = bot_name
        threading.Thread.__init__(self)
        self.start()

        # Required to allow everything to startup in the thread before we return.
        # Otherwise the caller will start calling our methods before the GUI's
        # variables are setup.
        time.sleep(1)

    def run(self):
        self.root=Tk()
        self.a = ChatInterface(self.root)
        self.root.geometry(window_size)
        self.root.title(self.bot_name)
        #self.root.after(1000, self.constant_update) # not recursion, it's a queued event call
        self.root.mainloop() # Blocks and never exits

    # def constant_update(self):
    #     print("(constant_update)")
    #     #a.send_message_insert("Insert this one!")
    #     #while True:
    #     user_input = self.a.entry_field.get()
    #     print("Current text input:", user_input)
    #     print("Queued messages:", self.a.user_input_history)
    #     self.root.after(1000, self.constant_update) # not recursion, it's a queued event call

    def input(self, text):
        '''Must give input(str) a string argument'''
        self.a.text_box.configure(state=NORMAL)
        self.a.text_box.insert(END, "MyBot: " + text + "\n")
        self.a.text_box.configure(state=DISABLED)
        self.a.text_box.see(END)
        # Spin for input.
        while len(self.a.user_input_history) == 0:
            time.sleep(.1)
        return self.a.user_input_history.pop()

    def print(self, *argv):
        '''Takes infinite arguments like Python's builtin print()'''
        text = ''
        for arg in argv:
            text = text + ' ' + str(arg)
        self.a.text_box.configure(state=NORMAL)
    ...
    def close(self):
        #print("control-c to quit or click the GUI x")
        #self.root.destroy()   # Don't call destroy, just call quit() for a clean break.
        self.root.quit()
        self.a.chatexit()
